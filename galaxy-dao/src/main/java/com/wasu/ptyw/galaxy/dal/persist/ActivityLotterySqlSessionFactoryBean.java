...
public class ActivityLotterySqlSessionFactoryBean implements BeanFactoryPostProcessor {

	
	private @Setter
	DataSource activitylotterydataSource;
	...
	// private @Setter Map<String, String> mappers;
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory bf)
		...
		try {
			Environment env = new Environment("activitylotterydataSource",
					new ManagedTransactionFactory(), activitylotterydataSource);
			...
			if (typeAliasesPackage != null && !typeAliasesPackage.isEmpty()) {
				conf.getTypeAliasRegistry().registerAliases(typeAliasesPackage,
						Object.class);
			}
	 ...
	 * mybatis 3.0.6
	 */
	private void registerBeans2(ConfigurableListableBeanFactory bf,
			Configuration conf) throws Exception {
		MapperRegistry mapperRegistry = getFieldValue(conf, "mapperRegistry");
		Set<Class<?>> knownMappers = getFieldValue(mapperRegistry,
				"knownMappers");
